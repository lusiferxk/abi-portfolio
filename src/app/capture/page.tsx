            ...
            <div className="columns-1 sm:columns-2 lg:columns-3 xl:columns-4 gap-4">
              {galleryItems.map((item, index) => {
                ...
                return (
                  <div
                    key={item.uniqueId}
                    className={`group relative overflow-hidden bg-zinc-100 cursor-pointer transition-all duration-500 ease-out rounded-xl w-full inline-block break-inside-avoid mb-4 ${getLayoutClasses(item.media.aspectRatio)} ${
                      isAnotherHovered ? "grayscale opacity-50 blur-[2px]" : "grayscale-0 opacity-100 blur-0"
                    }`}
